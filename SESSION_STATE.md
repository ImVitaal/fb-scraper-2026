...
The manager checkout contains the accepted local parser, capture, session-lifecycle, discovery-stop, and parity fixes through the current branch tip. The Phase 4G local wrapper now has atomic progress, strict resume-target validation, duplicate-target rejection, canonical identifier-union hashing, and an explicit external raw-root contract. It still has no accepted ten-Group APP receipt, real one-Group callback adapter, or CLI activation evidence.
...
| T3 Phase 4G ten-Group workflow | Local skeleton, gated | `OperatorBatchWorkflow` has 9 focused synthetic tests for atomic progress, strict resume validation, duplicate targets, canonical identifier unions, and external raw roots | After accepted Phase 4F evidence, wire the real one-Group resume callback and CLI/config path, then produce a controlled ten-Group receipt |
...
- Full suite: 244 passed; coverage 81.23% (2026-08-02).
...
3. Ten-Group execution remains gated. The local wrapper is hardened and tested, but it is still a skeleton, not a release receipt; it does not yet invoke the real one-Group resume path or the CLI/config surface.
