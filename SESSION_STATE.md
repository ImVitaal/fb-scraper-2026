...
| T2 Phase 4F one-Group APP proof | **REVISE** | The Group later showed `Joined` with recent visible activity/comments; the direct one-Group capture stopped at `interaction_failed` after four retries | Diagnose the capture interaction stop, then rerun the complete one-Group receipt without another Join |
...
- T2 later observed the same Group as `Joined` with recent visible activity/comments. The direct capture stop receipt is outside Git at `exports\8e1390a0-99a0-4aad-8f9d-7b968bb3f630.operator-receipt.json`; SHA-256 is `48184ae33d69263c816392aabf000655947a39e95a6a4526a61a5d03a8ff63ce`. It records `interaction_failed` after four retries, one Group, zero Posts, and zero Comments; cleanup found zero scanner-owned Chrome processes.
...
- The earlier manager discovery run found no supported joined Group candidate and stopped before target selection. The later read-only Group check exposed joined activity, but capture interaction failed before Posts/Comments were collected.
- These results are APP observations, not fixture proof of a successful collection.
...
1. Membership is now visible as `Joined`, but the direct one-Group capture stopped at `interaction_failed` after four retries with zero Posts and zero Comments.
2. The required one-Group APP receipt is open. Resume, offline replay, five-export parity, Comment reconciliation, and date-boundary evidence remain unproven against that controlled run.
...
The next live attempt starts only after confirming zero scanner-owned browser processes and clean external profile locks. Reuse the observed joined Group, investigate the interaction stop, and write a redacted receipt; do not submit another Join for the same Group.
